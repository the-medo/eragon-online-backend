...
func (server *ServiceMenus) CreateMenuItemPost(ctx context.Context, req *pb.CreateMenuItemPostRequest) (*pb.MenuItemPost, error) {
	violations := validateCreateMenuItemPostRequest(req)
	if violations != nil {
		return nil, e.InvalidArgumentError(violations)
	}

	authPayload, err := server.CheckMenuPermissions(ctx, req.GetMenuId(), &servicecore.ModulePermission{
	...
	rsp := &pb.MenuItemPost{
		...
		Post:       converters.ConvertPost(post),
	}
