...
RETURN QUERY EXECUTE format('
        ...
        WHERE
            ...
            ($4 IS NULL OR system_id = $4) AND
            ($5 IS NULL OR can_join = $5) AND
            ($6 IS NULL OR status = $6)
        ORDER BY %I %s %s
        OFFSET $7', _order_by, _order_direction, _limit_string)
    USING _is_public, _tags, _world_id, _system_id, _can_join, _status, _offset;
END
